...
def main():
    ...
    parser.add_argument("--count", type=int, default=10, help="要删除的消息数量（默认10条）")
    ...
    try:
        ...
        print("=" * 100)
        print("{:<24} {:<20} {:<10} {:<20} {:<15} {:<30}".format(
        ...
        for msg in all_messages:
            ...
            collection_name = "inputmessages" if "from_user" in msg else "outputmessages"
            message_ids_to_delete.append((message_id, collection_name))
            
            print("{:<24} {:<20} {:<10} {:<20} {:<15} {:<30}".format(
                message_id[:24], timestamp, direction, message_type, status, message_content))        
        print("=" * 100)
        ...
        for message_id, collection_name in message_ids_to_delete:
            try:
                ...
                result = mongo.delete_one(collection_name, {"_id": ObjectId(message_id)})
                if result > 0:
                    deleted_count += 1
        ...
        logger.info(f"删除操作完成，成功删除 {deleted_count}/{len(all_messages)} 条消息")

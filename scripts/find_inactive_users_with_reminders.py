                ...
                if active_reminders:
                    user_info = {
                        ...
                        "last_message_friendly": (
                            datetime.fromtimestamp(last_msg_time).strftime("%Y-%m-%d %H:%M:%S") 
                            if last_msg_time else "N/A"
                        ),
                        "days_since_last_message": (
                            (time.time() - last_msg_time) / (24 * 3600) 
                            if last_msg_time else float('inf')
                        ),
...
def print_results(results: List[Dict], days_threshold: int = 7):
    ...
    for i, user_info in enumerate(results, 1):
        ...
        print(f"     最后消息时间: {user_info['last_message_friendly']} ({user_info['days_since_last_message']:.1f}天前)")
        ...
        for j, reminder in enumerate(user_info['active_reminders'], 1):
            ...
            trigger_time_friendly = (
                ...
                if next_trigger_time else "N/A"
            )
            reminder_id = reminder.get('reminder_id', 'N/A')
            print(f"       [{j}] {title} (ID: {reminder_id}, 触发时间: {trigger_time_friendly})")
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description="查找指定天数内未发送消息但仍有未触发提醒的用户")

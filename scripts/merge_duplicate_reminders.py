...
def merge_duplicate_reminders(dry_run=True):
    ...
    try:
        ...
        print(f"{'[预览模式] ' if dry_run else ''}找到 {len(merge_candidates)} 组需要合并的提醒:")
        print("-" * 80)
        ...
        for i, (user_id, time_key, group) in enumerate(merge_candidates, 1):
            ...
            # 显示将删除的提醒
            reminders_to_delete = [r for r in group if r.get("reminder_id") != reminder_to_keep.get("reminder_id")]
        ...
        if dry_run:
            ...
            print("如需执行合并，请运行: python scripts/merge_duplicate_reminders.py --execute")
        else:
...
def main():
    ...
    parser.add_argument("--execute", action="store_true", help="执行合并操作（默认为预览模式）")

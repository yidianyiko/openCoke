...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.check_imports:
        _check_imports()
        print("OK: v2 parity probe imports resolved without runtime construction")
        return 0
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Run the v2 turn pipeline parity probe against the composed Coke "
    ...
    parser.add_argument(
        "--check-imports",
        action="store_true",
        help="Import the script and v2 symbols without building Settings or runtime.",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
...
def _check_imports() -> None:
    ...
    if not all(symbols):
        raise RuntimeError("v2 parity probe import check failed")
...
def _run_one_path(
    *,
    ...
    turn_id = f"v2-probe-{turn_index}-{uuid4().hex[:12]}"
    conversation_id = f"v2-probe-conversation-{account_id}"
...
def _cleanup_created_reminders(
    ...
    *,
    ...
    guard = RecordingGuard(f"v2-probe-cleanup-{uuid4().hex[:12]}")

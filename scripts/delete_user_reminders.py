...
def delete_user_reminders(user_id, dry_run=True):
    ...
    try:
        ...
        print(f"{'[预览模式] ' if dry_run else ''}找到用户 {user_id} 的 {len(reminders)} 个提醒:")
        print("-" * 80)
        ...
        if dry_run:
            ...
            print(f"如需执行删除，请运行: python scripts/delete_user_reminders.py --user {user_id} --execute")
        else:
            # 执行删除操作
            deleted_count = 0
...
def main():
    ...
    parser.add_argument("--user", type=str, help="用户ID", default="6945fc33f09e7d5a55ad9f29")

...
def check_output_messages():
    """检查输出消息"""
    now = int(time.time())
    messages = mongo.find_many("outputmessages", {
        "platform": "wechat",
        "from_user": to_user,
        "to_user": from_user,
    }, limit=10)
    ...
    for msg in messages:
        logger.info(f"  [{msg.get('status')}] {msg.get('message_type')}: {msg.get('message', '')[:50]}...")
    return messages
...
def check_reminders():
    ...
    for r in reminders:
        logger.info(f"  [{r.get('status')}] {r.get('title')} - 触发时间: {r.get('next_trigger_time')}")
    return reminders
...
async def test_dialog_loop():
    ...
    while True:
        try:
            ...
            # 检查是否创建了提醒
            reminders = reminder_dao.find_reminders_by_user(from_user, status="confirmed")
            if reminders:
...
async def main():
    ...
    parser.add_argument('--mode', choices=['auto', 'interactive', 'reminder', 'future'], 

...
此脚本用于分析将来会被触发的提醒，查找时间非常接近的提醒（5分钟内），
以确定是否还需要合并。

使用方法：
    python scripts/analyze_nearby_reminders_5min.py
"""
...
def analyze_nearby_reminders():
    ...
    try:
        ...
        # 分析每个用户的提醒
        nearby_candidates = []   # 时间接近的提醒组（5分钟内）
        ...
        for user_id, user_reminder_list in user_reminders.items():
            ...
            for i in range(len(sorted_reminders)):
                ...
                for j in range(i+1, len(sorted_reminders)):
                    ...
                    # 如果时间差在5分钟内（300秒），则加入组
                    if next_time - current_time <= 300:
                ...
                if len(nearby_group) > 1:
                    ...
                    for recorded_group in nearby_candidates:
                        if set(r.get("reminder_id") for r in recorded_group[4]) == set(r.get("reminder_id") for r in nearby_group):
        ...
        # 显示结果
        print("=" * 100)
        print("                          附近时间提醒分析报告 (5分钟内)")
        print("=" * 100)
        
        print(f"\n📊 总体统计:")
        print(f"  • 未来提醒总数: {len(reminders)}")
        print(f"  • 涉及用户数: {len(user_reminders)}")
        print(f"  • 时间接近组数: {len(nearby_candidates)} (5分钟内)")
        ...
        if nearby_candidates:
            ...
            for i, (user_id, min_time, max_time, time_diff, group) in enumerate(nearby_candidates, 1):
                ...
                print(f"    时间差: {time_diff} 秒 ({time_diff//60}分{time_diff%60}秒)")
...
def main():
    print("=" * 100)
    print("                   附近时间提醒分析工具 (5分钟内)")
    print("=" * 100)
    print("此工具将分析将来会被触发的提醒，查找时间非常接近的提醒（5分钟内）")

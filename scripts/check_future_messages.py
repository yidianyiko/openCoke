    ...
    for talker in talkers:
        ...
        try:
            ...
            if user:
                ...
                name = user.get("platforms", {}).get("wechat", {}).get("nickname", nickname)
...
def main():
    ...
    parser.add_argument("--all", action="store_true", help="显示所有有 future 配置的会话")
    ...
    parser.add_argument("--expired", action="store_true", help="仅显示已过期的消息（主动消息次数达到上限）")
    ...
    if args.all:
        ...
        # 仅显示已过期的
        query = {
            "conversation_info.future.status": "expired"
        }
    ...
    for conv in conversations:
        ...
        table_data.append([
            ...
            action[:30] + "..." if action and len(action) > 30 else (action or "无"),
            proactive_times,
            status or "pending"
        ])
    ...
    # 统计信息
    due_count = sum(1 for conv in conversations 
    ...
    pending_count = sum(1 for conv in conversations 
                       if conv.get("conversation_info", {}).get("future", {}).get("timestamp") 
                       and conv["conversation_info"]["future"]["timestamp"] > now
                       and conv["conversation_info"]["future"].get("status") != "expired")
    expired_count = sum(1 for conv in conversations 
                       if conv.get("conversation_info", {}).get("future", {}).get("status") == "expired")
    ...
    for i, conv in enumerate(conversations, 1):
        ...
        print(f"     时间状态: {get_time_status(future.get('timestamp'), future.get('status'))}")

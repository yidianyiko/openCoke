...
def analyze_reminder_status():
    ...
    try:
        ...
        # 查找异常状态
        triggered_non_recurring = list(dao.collection.find({
            "status": "triggered",
            "recurrence.enabled": False
        }))
        
        logger.info(f"发现 {len(triggered_non_recurring)} 个状态为 'triggered' 的非周期提醒")
...
def cleanup_reminder_status(dry_run=True):
    ...
    try:
        ...
        for reminder in triggered_non_recurring:
            logger.info(f"  - {reminder.get('title')} (ID: {reminder.get('reminder_id')})")
        
        logger.info("")
        
        if dry_run:
            logger.info("⚠️  DRY RUN 模式：不会实际修改数据")
            logger.info("如需实际执行，请运行: python scripts/cleanup_reminder_status.py --execute")
        else:
            # 执行清理

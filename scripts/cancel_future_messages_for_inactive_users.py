...
def get_conversations_by_user_ids(user_ids: List[str], conversation_dao: ConversationDAO) -> List[Dict]:
    """
    根据用户ID列表获取相关会话
...
def update_future_messages_to_expired(conversations: List[Dict], conversation_dao: ConversationDAO) -> Dict[str, int]:
    """
    将会话中的未来主动消息设置为过期状态
...
def main():
    ...
    try:
        # 获取与这些用户相关的会话
        conversations = get_conversations_by_user_ids(inactive_user_ids, conversation_dao)

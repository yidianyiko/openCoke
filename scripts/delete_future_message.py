...
def delete_future_message(conv_id: str, conversation_dao: ConversationDAO, show_detail: bool = True):
    """
    删除指定会话的未来主动消息
    ...
    if not future.get("timestamp") and not future.get("action"):
        print(f"  ⚠ 该会话没有配置未来主动消息")
        return False
    
    # 执行删除
    result = conversation_dao.update_conversation(conv_id, {

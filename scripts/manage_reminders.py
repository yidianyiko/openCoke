...
def print_reminder_table(reminders: List[dict], show_user: bool = True):
    ...
    for i, reminder in enumerate(reminders, 1):
        ...
        print()
        print(colorize(f"[{i}] ", Colors.BOLD) + colorize(title, Colors.CYAN + Colors.BOLD))
        print(f"    ID: {colorize(reminder_id, Colors.DIM)}")
        print(f"    状态: {format_status(status)}  |  周期: {format_recurrence(recurrence)}")
...
def list_reminders(status: Optional[str] = None, show_all: bool = False, user_id: Optional[str] = None):
    """列出提醒"""
    dao = ReminderDAO()
...
def reschedule_expired(reminder_ids: List[str] = None, dry_run: bool = True) -> int:
    ...
    try:
        ...
        for reminder in reminders:
            ...
            print(f"    {colorize('新时间:', Colors.GREEN)} {new_friendly} ({new_exact})")
        ...
        for update in updates:
            result = dao.collection.update_one(
                ...
                {"$set": {"next_trigger_time": update["new_time"], "updated_at": now_ts}}
            )
...
def delete_by_keyword(keyword: str, dry_run: bool = True) -> int:
    ...
    try:
        ...
        if dry_run:
            ...
            print(f"如需删除，请运行: python scripts/manage_reminders.py delete --keyword \"{keyword}\" --execute\n")
            return 0
...
示例:
    ...
    list_parser.add_argument("--all", "-a", action="store_true", help="显示所有状态的提醒")
    list_parser.add_argument("--status", "-s", help="按状态筛选 (confirmed/pending/triggered/completed/cancelled)")
    ...
    delete_parser.add_argument("--execute", action="store_true", help="实际执行删除（非预览模式）")
    ...
    reschedule_parser.add_argument("ids", nargs="*", help="指定要重新调度的提醒ID（可选，不指定则处理所有过期提醒）")
    reschedule_parser.add_argument("--execute", action="store_true", help="实际执行更新（非预览模式）")

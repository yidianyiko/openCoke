...
def add_langbot_platform_to_characters():
    ...
    for character in characters:
        ...
        langbot_platforms = [
            key for key in platforms.keys() if key.startswith("langbot_")
        ]
        ...
        if not langbot_platforms:
            ...
            logger.info(
                f"角色 {character_name} 已有 LangBot 平台信息: {langbot_platforms}"
            )

    logger.info(f"处理完成，共为 {updated_count} 个角色用户添加了 LangBot 平台信息")


def add_specific_platform_to_character(
    character_id: str, platform_name: str, platform_data: dict
):
    """
    为特定角色添加特定平台信息

...
def update_outdated_future_messages_to_expired(conversation_dao: ConversationDAO) -> Dict[str, int]:
    """
    将所有过时的未来主动消息设置为过期状态
    ...
    for conv in conversations:
        ...
        if future:
            ...
            if should_update:
                ...
                if success:
                    updated_count += 1
                    print(f"会话 {conv_id}: Future消息已设置为过期状态 (原计划时间: {datetime.fromtimestamp(timestamp) if timestamp else '未设置'}, 原状态: {status})")
                else:

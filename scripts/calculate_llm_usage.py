...
def count_conversation_messages(mongo_db, conversation_id):
    """统计会话中的消息数量"""
    # 统计会话中的输入消息数量
    input_count = mongo_db.count_documents("inputmessages", {"conversation_id": conversation_id})
    
    # 统计会话中的输出消息数量
    output_count = mongo_db.count_documents("outputmessages", {"conversation_id": conversation_id})
...
def main():
    ...
    try:
        ...
        for user in users:
            ...
            wechat_nickname = wechat_info.get("nickname", "N/A") if isinstance(wechat_info, dict) else "N/A"
        ...
        # 计算平均调用次数（所有用户）
        avg_total_calls = (total_input_messages + total_output_messages) / total_users if total_users > 0 else 0
        ...
        # 计算平均调用次数（过滤后用户）
        avg_total_calls_filtered = (total_input_messages_filtered + total_output_messages_filtered) / filtered_user_count if filtered_user_count > 0 else 0
        avg_input_calls_filtered = total_input_messages_filtered / filtered_user_count if filtered_user_count > 0 else 0
        avg_output_calls_filtered = total_output_messages_filtered / filtered_user_count if filtered_user_count > 0 else 0
        ...
        print(f"过滤后总消息数: {total_input_messages_filtered + total_output_messages_filtered}")
        ...
        # 显示所有用户信息（按要求列出所有用户）
        print("\n所有用户详细统计:")
        print("{:<24} {:<20} {:<20} {:<20} {:<10} {:<10} {:<10} {:<12} {:<12} {:<12}".format(
            "用户ID", "用户名", "微信昵称", "创建时间", "输入消息", "输出消息", "总计", "日均", "周均", "活跃天数"))
        print("-"*160)
        
        for stat in user_stats:
            # 格式化活跃天数
            active_days_str = f"{stat['active_days']:.1f}" if stat['active_days'] > 0 else "N/A"
            # 格式化创建时间
            creation_time_str = format_timestamp(stat['creation_time']) if stat['creation_time'] else "N/A"
            
            print("{:<24} {:<20} {:<20} {:<20} {:<10} {:<10} {:<10} {:<12.2f} {:<12.2f} {:<12}".format(
                stat["user_id"][:24],
                stat["name"][:20],
        ...
        total_tokens_filtered = (total_input_messages_filtered + total_output_messages_filtered) * 1000

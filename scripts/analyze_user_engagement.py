...
def get_user_first_message_time(mongo_db, user_id):
    ...
    try:
        # 查找用户最早的一条输入消息
        input_cursor = mongo_db.db["inputmessages"].find({"from_user": user_id}).sort("input_timestamp", 1).limit(1)
        input_msgs = list(input_cursor)
        input_msg = input_msgs[0] if input_msgs else None
        
        # 查找用户最早的一条输出消息
        output_cursor = mongo_db.db["outputmessages"].find({"to_user": user_id}).sort("input_timestamp", 1).limit(1)
        output_msgs = list(output_cursor)
        output_msg = output_msgs[0] if output_msgs else None
        
        # 获取最早的时间戳
        input_time = input_msg.get("input_timestamp", float('inf')) if input_msg else float('inf')
        output_time = output_msg.get("input_timestamp", float('inf')) if output_msg else float('inf')
        
        first_time = min(input_time, output_time)
        return first_time if first_time != float('inf') else None
    except Exception as e:
        # 如果出现异常，返回None
        return None
...
def calculate_average_usage(user_data, current_time):
    ...
    for user in user_data:
        ...
        if user["first_message_time"] and user["last_message_time"]:
            active_duration_days = (user["last_message_time"] - user["first_message_time"]) / 86400
            # 至少活跃1天才能计算平均值
            if active_duration_days >= 1:
                ...
                user["avg_weekly_usage"] = user["total_count"] / active_duration_weeks if active_duration_weeks > 0 else 0
                
                # 计算平均每月调用量
                active_duration_months = active_duration_days / 30  # 简化按30天一个月计算
                user["avg_monthly_usage"] = user["total_count"] / active_duration_months if active_duration_months > 0 else 0
            else:
...
def analyze_user_engagement():
    ...
    try:
        ...
        print("所有用户基本信息")
        print("="*160)
        print("{:<24} {:<20} {:<20} {:<20} {:<20} {:<10} {:<10} {:<10} {:<10} {:<10} {:<10}".format(
            "用户ID", "用户名", "创建时间", "首次活动时间", "最后活动时间", "输入消息", "输出消息", "总计", "日均", "周均", "月均"))
        print("-"*160)
        
        for user in user_data:
            creation_time_str = format_timestamp(user['creation_time']) if user['creation_time'] else "N/A"
            first_message_str = format_timestamp(user['first_message_time']) if user['first_message_time'] else "N/A"
            last_message_str = format_timestamp(user['last_message_time']) if user['last_message_time'] else "N/A"
            
            print("{:<24} {:<20} {:<20} {:<20} {:<20} {:<10} {:<10} {:<10} {:<10.2f} {:<10.2f} {:<10.2f}".format(
                user["user_id"][:24],
                user["name"][:20],
        ...
        for user in user_data:
            ...
            if user["creation_time"]:
                ...
                daily_registration[creation_date] = daily_registration.get(creation_date, 0) + 1
            
            # 统计每日活跃用户数
            if user["last_message_time"]:
                last_activity_date = datetime.fromtimestamp(user["last_message_time"]).date()
                daily_activity[last_activity_date] = daily_activity.get(last_activity_date, 0) + 1
        ...
        ongoing_engagement_users = category_counts.get("Occasional", 0) + category_counts.get("Regular", 0) + category_counts.get("Highly Active", 0)
        non_active_users = category_counts.get("Non-active", 0)
        
        print(f"从未使用服务的用户 (Non-active): {non_active_users} ({non_active_users/total_users*100:.2f}%)")
        print(f"一次性用户 (One-time): {single_session_users} ({single_session_users/total_users*100:.2f}%)")
        print(f"持续参与用户 (Occasional/Regular/Highly Active): {ongoing_engagement_users} ({ongoing_engagement_users/total_users*100:.2f}%)")
        ...
        for week in sorted(users_by_registration_week.keys()):
            ...
            for user in users_by_registration_week[week]:
                ...
                if user["last_message_time"]:
                    days_since_last_activity = (current_time - user["last_message_time"]) / 86400
                    if days_since_last_activity <= 7:  # 一周内有活动
                        active_users += 1
            
            active_rate = (active_users / registered_users * 100) if registered_users > 0 else 0
            week_label = f"本周" if week == 0 else f"{week}周前"
            explanation = "当前仍活跃用户占比（最近7天内有活动）"
            print(f"{week_label:6} | {registered_users:10} | {active_users:10} | {active_rate:6.1f}% | {explanation}")
        ...
        for segment, stats in segment_stats.items():
            ...
            print(f"{segment:12} | {registered:10} | {active:14} | {retention_rate:6.1f}% | 最近7天内有活动的用户占比")
        ...
        if active_users:
            avg_daily = sum(u["avg_daily_usage"] for u in active_users) / len(active_users)
            avg_weekly = sum(u["avg_weekly_usage"] for u in active_users) / len(active_users)
            avg_monthly = sum(u["avg_monthly_usage"] for u in active_users) / len(active_users)
            ...
            for category, users in categories.items():
                ...
                avg_monthly_cat = sum(u["avg_monthly_usage"] for u in users) / len(users)
                print(f"  {category}: 日均 {avg_daily_cat:.2f}, 周均 {avg_weekly_cat:.2f}, 月均 {avg_monthly_cat:.2f}")
        else:
            print("没有活跃用户数据可供统计")

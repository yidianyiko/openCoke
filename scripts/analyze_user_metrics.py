...
def analyze_cost_metrics(users: List[Dict], mongo_db: MongoDBBase) -> Dict:
    ...
    avg_interactions_per_user = (
        total_interactions / len(active_users) if active_users else 0
    )
    avg_daily_per_user = (
        sum(u["daily_avg"] for u in active_users) / len(active_users)
        if active_users
        else 0
    )
    ...
    for week_start_date in sorted(cohorts.keys(), reverse=True):
        ...
        for user_data in cohort_users:
            ...
            for day in retention_days:
                ...
                # 累积留存：第N天及之后是否有任何活跃
                has_activity_after = any(d >= target_date for d in active_dates)
        ...
        for day in retention_days:
            day_key = f"day{day}"
            if retention_possible[day_key]:
                cumulative_rates[day_key] = (
                    cumulative_retention[day_key] / cohort_size
                ) * 100
            else:
                cumulative_rates[day_key] = None

        avg_active_days = (
            total_active_days_in_cohort / cohort_size if cohort_size > 0 else 0
        )
...
def analyze_user_distribution(users: List[Dict], mongo_db: MongoDBBase) -> Dict:
    ...
    return {
        ...
        "tiers": tiers,
        "user_data": sorted(
            user_data_list, key=lambda x: x["input_count"], reverse=True
        ),
    }
...
def analyze_daily_trend(users: List[Dict], mongo_db: MongoDBBase) -> Dict:
    ...
    interaction_values = [
        d["interactions"] for d in trend_data if d["interactions"] > 0
    ]
    ...
    summary = {
        ...
        "avg_interactions": (
            statistics.mean(interaction_values) if interaction_values else 0
        ),
...
def print_cohort_report(cohort_data: Dict):
    ...
    print(f"\n【精确留存率】（第N天当天是否活跃）")
    print(
        f"{'注册周':<18} {'用户数':<8} {'Day1':<8} {'Day3':<8} {'Day5':<8} {'Day7':<8} {'Day14':<8} {'Day30':<8} {'平均活跃天数':<12}"
    )
    print("-" * 120)
    ...
    print(f"\n【累积留存率】（第N天及之后是否有任何活跃）")
    print(
        f"{'注册周':<18} {'用户数':<8} {'Day1+':<10} {'Day3+':<10} {'Day7+':<10} {'Day14+':<10} {'Day30+':<10}"
    )
    print("-" * 120)
    ...
    print(f"\n【留存绝对数字】")
    print(
        f"{'注册周':<18} {'用户数':<8} {'Day1':<10} {'Day3':<10} {'Day5':<10} {'Day7':<10} {'Day14':<10} {'Day30':<10}"
    )
    print("-" * 120)
...
def print_distribution_report(dist_data: Dict):
    ...
    print(f"\n【用户分层概览】")
    print(
        f"  轻度用户 (日均<5次):   {tiers.get('light', 0):>6} 人 ({tiers.get('light', 0)/total_users*100 if total_users else 0:>5.1f}%)"
    )
    print(
        f"  中度用户 (日均5-20次): {tiers.get('medium', 0):>6} 人 ({tiers.get('medium', 0)/total_users*100 if total_users else 0:>5.1f}%)"
    )
    print(
        f"  重度用户 (日均20+次):  {tiers.get('heavy', 0):>6} 人 ({tiers.get('heavy', 0)/total_users*100 if total_users else 0:>5.1f}%)"
    )
    ...
    max_count = (
        max(d["count"] for d in dist_data["distribution"])
    ...
    for pct in [1, 5, 10, 20, 50]:
        ...
        print(
            f"  Top {pct:>2}% 用户 ({users:>3}人) 贡献交互: {interactions:>6,} ({percentage:>5.1f}%)"
        )
...
def print_trend_report(trend_data: Dict):
    ...
    print(f"\n【每日明细】")
    print(
        f"{'日期':<12} {'星期':<6} {'DAU':<8} {'交互数':<10} {'DAU趋势':<40} {'交互趋势'}"
    )
    print("-" * 100)
    ...
    for d in trend:
        ...
        int_bar_len = (
            int((d["interactions"] / max_interactions) * 20)
...
def export_user_summary_csv(users: List[Dict], mongo_db: MongoDBBase, output_path: str):
    ...
    for user in users:
        ...
        # 会话信息
        conversation = mongo_db.find_one("conversations", {"talkers.id": user_id})
        proactive_times = 0
        has_future = False
        if conversation:
...
def export_daily_activity_csv(
    users: List[Dict], mongo_db: MongoDBBase, output_path: str
):
    """
    ...
    for user in users:
        ...
        for msg in output_messages:
            output_ts = msg.get("expect_output_timestamp") or msg.get(
                "handled_timestamp"
            )
            if not output_ts:
                continue
        ...
        for date_str, stats in sorted(daily_stats.items()):
            ...
            if stats["first_msg_time"]:
                try:
                    first_hour = datetime.fromtimestamp(
                        stats["first_msg_time"]
                    ).strftime("%H:%M")
                except (ValueError, OSError, TypeError):
                    pass
            if stats["last_msg_time"]:
                try:
                    last_hour = datetime.fromtimestamp(stats["last_msg_time"]).strftime(
                        "%H:%M"
                    )
                except (ValueError, OSError, TypeError):
                    pass
            ...
            avg_handle = (
                statistics.mean(stats["handle_times"]) if stats["handle_times"] else 0
            )
...
def main():
    ...
    # 如果没有指定任何选项，则显示全部
    show_all = not (
        args.cost or args.cohort or args.distribution or args.trend or args.export_csv
    )
    ...
    try:
        ...
        if args.export_csv:
            ...
            daily_count = export_daily_activity_csv(
                users, mongo_db, daily_activity_path
            )

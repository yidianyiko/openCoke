...
def fetch_conversation_data(user_id: str, character_id: str):
    """获取用户和角色之间的对话数据"""
    conversations = conversation_dao.find_conversations_by_user(user_id, platform="wechat")
    for conv in conversations:
        for talker in conv.get("talkers", []):
            if talker.get("id") == character_id or str(talker.get("id")) == str(character_id):
                return conv
    return None
...
def fetch_relation_data(user_id: str, character_id: str):
    """获取用户和角色之间的关系数据"""
    relation = mongo.find_one("relations", {"uid": str(user_id), "cid": str(character_id)})
    return relation
...
async def run_diagnostic():
    ...
    character_nickname = character.get("platforms", {}).get("wechat", {}).get("nickname", "未知")
    character_wxid = character.get("platforms", {}).get("wechat", {}).get("id", "未知")
    logger.info(f"角色ID: {character_id}, 昵称: {character_nickname}, wxid: {character_wxid}")
    ...
    if not relation:
        ...
        logger.info(f"关系数据: closeness={relation.get('relationship', {}).get('closeness', 'N/A')}")
            ...
            for i in range(0, min(len(chat_history), 50), 5):
                ...
                if not chunk_result:
                    ...
                    for j, msg in enumerate(chunk):
                        ...
                        logger.error(f"   [{i+j}] {msg.get('from_nickname', '未知')}: {content}...")
    else:
...
"""
        ...
        if not results["3_relation"]:
            ...
            for field_name, field_value in [
                ("relationship.description", relation.get('relationship', {}).get('description', '')),
                ("user_info.realname", relation.get('user_info', {}).get('realname', '')),
                ("user_info.hobbyname", relation.get('user_info', {}).get('hobbyname', '')),
                ("user_info.description", relation.get('user_info', {}).get('description', '')),
                ("character_info.attitude", relation.get('character_info', {}).get('attitude', '')),
                ("character_info.shortterm_purpose", relation.get('character_info', {}).get('shortterm_purpose', '')),
                ("character_info.longterm_purpose", relation.get('character_info', {}).get('longterm_purpose', '')),
            ]:
                if field_value:

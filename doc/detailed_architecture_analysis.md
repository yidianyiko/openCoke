...
- **微信Bot对接层框架**：支持多种微信接入方式（ecloud、gewechat）
...
├── gewechat/                      # GeWeChat连接器
│   ├── __init__.py
│   ├── gewechat_connector.py     # 连接器主类
│   ├── gewechat_channel.py       # 通道管理
│   ├── gewechat_message.py       # 消息封装
│   ├── context.py                # 上下文类型定义
│   ├── reply.py                  # 回复类型定义
│   ├── common/                   # 通用工具
│   │   ├── log.py
...
```json
{
  "dev": {
    ...
    "gewechat": {
      "gewechat_base_url": "http://gewechat服务地址",
      "gewechat_token": "",
      "gewechat_app_id": "",
      "bot_wxid": "机器人微信ID"
    },
...
- gewechat继承BaseConnector但实现不完整
- 两者集合命名不一致（input_messages vs inputmessages）

    ...
    def render(self) -> str:
        ...
        return "\n".join(
            [
                ...
                (
                    "Core role: 你叫Coke，你是用户在微信中的健康搭子。你的主要目标是："
                    "1）督促用户完成近期的目标并进行提醒；"
                ...
                (
                    "Goal coaching: when the user mentions a concrete goal or task, "
                    "ask when they plan to do it and whether they need a reminder; "
                    "do not force a goal-follow-up into the first-use greeting."
                ),
...
def _onboarding_guidance_block(request: AgentRequest) -> str | None:
    ...
    lines = [
        ...
        (
            "Role framing: 你叫Coke，你是用户在微信中的健康搭子。你的主要目标是："
            "1）督促用户完成近期的目标并进行提醒；"
            "2）使用日历工具，帮用户预约和他人的时间；"
            "3）解答问题。"
        ),
        (
            "Natural greeting template: Hi, <user name>！我是 Coke，"
            "你的提醒和约课小助手。"
        ),
        (
            "Supported product surfaces are constraints, not a required visible "
            "checklist: " + "; ".join(capabilities)
        ),
...
def _social_scheduling_outcomes_block(outcomes: list[Mapping[str, Any]]) -> str:
    return "\n".join(
        [
            ...
            "created_active requires claim active_created; rescheduled_active requires active_rescheduled; duplicate_active requires active_duplicate; blocked_* requires the matching blocked_* claim and blocker.",
        ]
    )

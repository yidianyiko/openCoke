...
Created: 2026-05-28  
Updated: 2026-06-12 (added rapid consecutive inbound message handling as a daily conversation reliability requirement)
...
|---|---|---|---|---|---|---|
| 1 | Support journey: registration/login and account access | Individual user | Registration page, login page, account access status page, subscription page, public explanation page | Create a web account, log in, establish a session; know the next step when subscription or access status blocks use | Support registration, login, current user query, session/token; currently email auth is disabled, so registration directly creates an email-verified logged-in account, verification email resend is disabled, and forgot/reset password is disabled; show users subscription/access status and the next step; enforce account access before channel connection, inbound assistant processing, and calendar import; public explanation, FAQ, demo, privacy, terms, and subscription pages may serve as product understanding, recovery, and compliance support | Current product contract |
...
| Conversation ordering and stale-reply safety | Daily conversation; personal reminders | When multiple messages arrive in the same conversation, the system must preserve the correct user-intent order, including rapid consecutive messages separated by only milliseconds. Older in-progress context must not continue to act as the latest user intent after a newer message changes the conversation. Newer inbound messages may interrupt or supersede older in-progress work, but accepted messages must not be lost and the current input window must not be stranded. The system must not send duplicate replies or stale replies for work the user has already superseded. Partial, waiting, or already sent replies must be remembered well enough to avoid confusing repetition, and failures must remain observable. A visible waiting reply is never a final conversation outcome: when a response is still required, the system must later reach a final text reply, intentional no-reply, observable failure, or recovery reply for the complete current input window. |
...
- Account access is a current product gate, not only a display status. Access can be denied because subscription access is inactive or the account is suspended. Email verification remains a stored status, but current clean production has email auth disabled and treats the email verification gate as satisfied.
...
- Current clean production disables email-auth flows through `COKE_EMAIL_AUTH_ENABLED=0`: web registration directly creates a logged-in, email-verified account; verification email resend and forgot/reset password are unavailable.

User journey:

1. The individual user opens the registration or login entry point.
2. The user completes registration or login.
3. The system establishes a session/token and can return the current user identity and access status directly; email verification is not required while email auth is disabled.
4. The user completes subscription or renewal when necessary.
5. Subsequent channel connection, reminder, friendship, shared reminder, and calendar import usage can continue only when account access allows that surface.
...
- Showing or sending the recovery step for subscription renewal or suspended-account support.
...
- When the same user sends multiple messages in the same conversation in a rapid burst, including millisecond-scale consecutive messages, all accepted messages must remain part of the ordered conversation input. Newer inbound messages may interrupt older in-progress work, but the interruption must not lose messages, leave the conversation waiting forever, or prevent the system from reaching the correct final outcome for the complete current input window.
...
3. The system binds each accepted inbound message to a trusted account/customer context.
4. If several messages arrive very quickly in the same conversation, the system preserves their order and processes the complete current input window rather than silently dropping or forgetting any accepted message.
5. The system executes a single-Agent turn for the current input window.
6. The system decides whether the user message or message group needs a response.
7. If a reply is needed, the user receives an immediate text reply. If processing times out but is still ongoing, the user first receives a visible waiting text, and the final text reply is later delivered asynchronously.
8. If the system explicitly produces intentional no-reply, the system sends no user-visible message, but this result should be distinguished from system failure.
9. If processing fails, the error/failure must be observable and must not be mixed with intentional no-reply.
10. If this is a first-use scenario, the system may enter the onboarding guidance branch.
...
- Handling rapid consecutive inbound messages in the same conversation without losing accepted messages, duplicating replies, sending stale replies, or leaving the input window open indefinitely.
- Allowing newer inbound messages to interrupt older in-progress work before the older work has reached its final user-visible outcome.
...
- A visible waiting text is not a terminal outcome. After a waiting text for an input window, the system must still reach a final text reply, intentional no-reply, observable failure, or recovery reply.
- Rapid message interruption is allowed only as a stale-work safety mechanism. It must not strand non-terminal work, advance the wrong input boundary, or drop accepted user content.
...
- No matter how the system processes messages internally, the user-visible semantics must remain: accepted messages are received, ordered, and processed as the current input window; when a reply is needed, it is replied to in text; timeout produces a visible waiting text, and the final text reply can be delivered asynchronously; when the model intentionally no-replies, no user-visible message is sent; stale or duplicate replies should not appear after a newer user intent supersedes older work.
- This requirement defines functional correctness for bursty input handling, not a fixed latency SLO or a requirement to reply separately to every individual message in a burst.
...
- Shared reminder duration is inferred from explicit duration, explicit time range, or LLM-estimated task duration. The system must not silently apply a fixed 15-minute duration when conversation input omits duration.
- Receiver conflict is a hard pre-creation constraint. The system checks each receiver for conflicts in the time interval corresponding to the reminder duration inferred for the task. If any receiver has a conflict, the system does not create the shared reminder. It should explain who has a conflict and who is available, then ask the creator to adjust time or participants.
...
7. The system confirms the shared reminder's title or activity content, trigger time, and inferred or explicitly set duration. Time is interpreted according to the user's global default timezone.
...
- Supporting duration from explicit user input, explicit time range, or LLM-estimated task duration.
- Before creation, checking each receiver conflict in the time interval corresponding to the inferred reminder duration.
...
- Conversation-created reminder duration is inferred from explicit duration, explicit time range, or LLM-estimated task duration. The system must not silently apply a fixed 15-minute duration when conversation input omits duration. Time is interpreted according to the user's global default timezone. Independent timezone per reminder is not supported.
...
4. If the user explicitly gives a duration, the system uses that setting. If not, the conversation path estimates a reasonable duration from the task content and context instead of applying a fixed default.
...
- Duration is a reminder duration field, user-configurable and conversation-inferred when omitted. For personal reminders, it does not change whether or when the reminder triggers, nor completion or deletion. It is mainly used for display and for the receiver conflict time-window calculation in shared reminders.
...
- A messaging-first account has no password. Its only web authentication path is a one-time login claim. A web-first account uses email and password. Forgot/reset password is currently unavailable while email auth is disabled. The two credential types do not cross.

...
def test_create_update_archive_delete_card_records_events():
    ...
    archived = runtime.archive_card(owner_id="owner-1", card_id=card.id, actor_type="user", actor_id="user-1")
    deleted = runtime.delete_card(owner_id="owner-1", card_id=card.id, actor_type="user", actor_id="user-1")
...
def test_search_excludes_deleted_and_other_owner_cards():
    ...
    runtime.delete_card(owner_id="owner-1", card_id=own.id, actor_type="user", actor_id="u1")
...
Create `/data/projects/coke-memo-runtime/memo_runtime/storage/memory.py` with deterministic id counters, owner-scoped maps, idempotency replay, and `events` as a public tuple/list for tests.

- [ ] **Step 4: Implement contract request classes and CRUD**

Modify `/data/projects/coke-memo-runtime/memo_runtime/contract.py` to define:
...
Also define `UpdateMemoCardRequest`, `SearchMemoCardsRequest`, and implement
`create_card`, `update_card`, `archive_card`, `delete_card`, `get_card`, and
`search_cards`. The first search implementation may score keyword/tag matches
without embeddings.
...
from memo_runtime.config import MemoRuntimeConfig
from memo_runtime.contract import CreateMemoCardRequest, GetMemoReviewQueueRequest, MemoRuntimeContract, SearchMemoCardsRequest
from memo_runtime.storage.memory import InMemoryMemoStorage
...
- [ ] **Step 3: Wire review request classes into the contract**
...
Implement `get_review_queue()` by calling `review.build_review_queue()`.
...
- [ ] **Step 2: Implement proposal request classes and transitions**

Modify `/data/projects/coke-memo-runtime/memo_runtime/contract.py` to add
`CreateMemoProposalRequest`, `AcceptMemoProposalRequest`, and
`RejectMemoProposalRequest`, then implement `create_proposal`,
...
Coke. Use parameterized psycopg queries only.
...
## Task 7: Add Coke Adapter Boundary
...
class MemoCapabilityPort:
    def __init__(self, *, contract_factory: Callable[[Any], Any]) -> None:
    ...
    async def run(self, input_message: str, run_context: Any, args: dict[str, Any]) -> CapabilityResult:
        ...
        owner_id = str(getattr(run_context, "user_id", "") or "").strip()
        ...
        return CapabilityResult(
            name="memo",
            ok=True,
            content={"hits": getattr(result, "hits", result.get("hits", []))},
...
## Task 8: Add Submodule/Dependency Wiring And Docs
...
- [ ] **Step 3: Document product surface discovery**
...
```bash
git add .gitmodules memo-runtime docs/ARCHITECTURE.md docs/product-specs/FEATURE_TREE.md artifacts/evidence/2026-05-17-memo-runtime-contract.md

...
**Architecture:** ChannelReachability owns only `channel`, `delivery_route`, and `delivery_attempt`; it reads IdentityAccess-owned `channel_identity` through service methods and never writes that table. The first implementation uses dataclasses plus a protocol-friendly in-memory repository so lifecycle, route resolution, and provider idempotency are locked down before a SQLAlchemy repository slice maps the same boundary to the existing clean schema. Provider adapters are edge modules behind one canonical protocol, and webhook routes return structured accepted/identity facts only.
...
Expected: output includes backend or repo-OS surfaces. Keep the output in the handoff notes for the implementation session.
...
class WhatsAppEvolutionAdapter:
    ...
    def normalize_inbound(self, payload: Mapping[str, object]) -> NormalizedInbound:
        return NormalizedInbound(
            ...
            provider_subject=str(payload["sender"]),
            text=str(payload.get("text", "")),
            raw_event_id=str(payload["message_id"]),
...
class WeChatPersonalAdapter:
    ...
    def normalize_inbound(self, payload: Mapping[str, object]) -> NormalizedInbound:
        return NormalizedInbound(
            provider_type=self.provider_type,
            provider_subject=str(payload["wxid"]),
            text=str(payload.get("text", "")),
            raw_event_id=str(payload["message_id"]),
            received_at=self._now(),
            pairing_code=_optional_string(payload.get("pairing_code")),
...
class WeChatECloudAdapter:
    ...
    def normalize_inbound(self, payload: Mapping[str, object]) -> NormalizedInbound:
        return NormalizedInbound(
            provider_type=self.provider_type,
            provider_subject=str(payload["sender_id"]),
            text=str(payload.get("content", "")),
            raw_event_id=str(payload["msg_id"]),
            received_at=self._now(),
            pairing_code=_optional_string(payload.get("pairing_code")),
            payload=dict(payload),
        )
...
class LinqAdapter:
    ...
    def normalize_inbound(self, payload: Mapping[str, object]) -> NormalizedInbound:
        return NormalizedInbound(
            provider_type=self.provider_type,
            provider_subject=str(payload["from"]),
            text=str(payload.get("body", "")),
            raw_event_id=str(payload["id"]),
            received_at=self._now(),
            pairing_code=_optional_string(payload.get("pairing_code")),
            payload=dict(payload),
        )
...
def test_reconnect_reuses_route_key_safely(identity_service, reachability):
    ...
    assert second_route.route_key == "whatsapp_evolution:whatsapp:+15555550123"
    assert service.get_status(account.id).reachable is True
...
- [ ] **Step 3: Add read-only IdentityAccess method when absent**

Modify `coke/domains/identity_access/service.py` by adding this method immediately after `can_remove_channel_identity`:
...
Expected: this method reads IdentityAccess-owned data and does not add any channel identity writer.

- [ ] **Step 4: Add channel repository boundary**
...
- [ ] **Step 5: Add status and webhook result models**
...
- [ ] **Step 6: Add the ChannelReachability service**
...
class ChannelReachabilityService:
    def __init__(
        ...
        self._now = now or (lambda: datetime.now(UTC))
        self._id_factory = id_factory or (lambda prefix: f"{prefix}_{datetime.now(UTC).timestamp():.6f}")
    ...
    def resolve_route(self, account_id: str) -> DeliveryRoute:
        ...
        route_key = f"{channel.provider_type}:{identity.provider_subject}"
        route = DeliveryRoute(
...
- [ ] **Step 8: Export service class from the package initializer**
...
- [ ] **Step 9: Run the service tests after the package export**
...
- [ ] **Step 10: Commit service lifecycle**

Run:

```bash
git add coke/domains/identity_access/service.py coke/domains/channel_reachability tests/unit/coke/channel_reachability/test_channel_reachability_service.py
...
def test_provider_webhook_normalizes_and_returns_structured_identity_facts_only():
    ...
    response = client.post(
        "/api/provider-webhooks/whatsapp_evolution/inbound",
        json={
            "message_id": "wa_msg_1",
            "sender": "whatsapp:+15555550123",
            "text": "hello",
...
def create_provider_webhook_blueprint(reachability_service, providers) -> Blueprint:
    blueprint = Blueprint("provider_webhooks", __name__, url_prefix="/api/provider-webhooks")
    ...
    @blueprint.post("/<provider_type>/inbound")
    def inbound(provider_type: str):
...
class FakeReachabilityService:
    ...
    def send_text(self, account_id, text, idempotency_key, turn_id=None, message_id=None):
        self.calls.append(
            (
                "send_text",
                {
        ...
        return SimpleNamespace(
            ...
            provider_idempotency_key=idempotency_key,
            status="sent",
            provider_message_id="provider:idem_1",
            error_code=None,
            attempted_at=NOW,
            delivered_at=None,
        )
...
def create_channel_blueprint(reachability_service) -> Blueprint:
    ...
    @blueprint.post("/send-text")
    def send_text():
        payload = _json_payload()
        attempt = reachability_service.send_text(
            account_id=_body_field(payload, "account_id"),
            text=_body_field(payload, "text"),
            idempotency_key=_body_field(payload, "idempotency_key"),
            ...
            jsonify(
                {
                    ...
                    "delivered_at": (
                        attempt.delivered_at.isoformat() if attempt.delivered_at is not None else None
                    ),
                }
            ),
            202,
...
Expected: `suggest-verification` prints the relevant surface commands; `review-trigger` prints a non-blocking risk report. Copy the risk report summary into the handoff.
...
```bash
git status --short
git add coke/domains/channel_reachability coke/providers coke/api/channel_routes.py coke/api/provider_webhooks.py coke/app.py coke/domains/identity_access/service.py tests/unit/coke/channel_reachability

...
- [x] Add Track B waiting-delivery tests:
...
- [x] Run the focused Track B tests and confirm they fail for missing behavior:
...
- [x] Add Track F notification lifecycle tests:
...
- [x] Run the focused Track F tests and confirm they fail for missing behavior:
...
- [x] Add Track G onboarding tests:
  ...
  - `first_guidance_sent_at` stamps only after a committed visible final
    ...
    paths.
- [x] Run the focused Track G tests and confirm they fail for missing behavior:
...
- [x] Extend delivery diagnostics data structures:
  ...
  - measure adapter latency and persist source/intent/retry/trace/container/token
    diagnostics.
- [x] Add context-token observation support in conversation runtime so waiting
  sends can record token source and age without mutating turn state.
- [x] Centralize waiting delivery:
...
- [x] Make `TurnRunner` settle notification recipients on every terminal
...
- [x] Add SocialScheduling notification-recipient reconciler:
...
- [x] Inject onboarding guidance facts from the identity gate and existing
  configured settings only when `onboarding_guidance_required` is true.
- [x] Render a conditional `onboarding_guidance` prompt block in the Interaction
  Agent with only current product capabilities.
- [x] Stamp first guidance only after committed visible final onboarding reply
...
- [x] Update `docs/ARCHITECTURE.md` for delivery attempt diagnostics, waiting
  retry boundary, notification terminal settlement/reconciler, and onboarding
  stamp semantics.
- [x] Run focused tests for all touched behavior and make them pass.
- [x] Run required unit scope:
...
- [x] Run diff-aware routing:
  `zsh scripts/suggest-verification --base HEAD~1`.
- [x] Run risk trigger report:
  `zsh scripts/review-trigger --base HEAD~1`.
- [x] Follow the suggested verification surface and record real output.
- [x] Commit implementation/docs/test changes in small coherent commits with

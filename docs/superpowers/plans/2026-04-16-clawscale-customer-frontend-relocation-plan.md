...
- [x] Record any remaining `/coke/*` generic links that plan 6 must delete outright.
  ...
  - Frontend token follow-up note: the Coke-named channel wrapper now prefers `customer_token` but falls back to `coke_user_token` for neutral channel requests, and legacy-only auth successes clear stale customer session state before routing into `/channels/wechat-personal`.

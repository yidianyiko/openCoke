...
| 4 | 核心旅程：个人提醒 | 个人用户 | 对话中的 reminder intent，或提醒管理页 | 创建、查看、编辑、完成、取消个人提醒，并在到点时收到提醒 | 提醒必须 owner-scoped；支持自然语言和 UI/API 创建、查看、编辑、完成、取消；支持相对时间、具体时间、时区、重复规则；到点后进入 Interaction LLM，由 LLM 知道这是系统提醒，并用符合角色语气的文字提醒告知用户；一次性提醒完成，重复提醒推进下一次触发 | 已确认核心 |
...
| 提醒识别与个人提醒 CRUD | 个人提醒 | 用户可通过自然语言或提醒管理页创建、查看、编辑、完成、取消个人提醒；系统需理解相对时间、具体时间、时区、重复规则，并保持 owner-scoped。 |
| 提醒触发与周期提醒 | 个人提醒；共享提醒投影 | 到点提醒必须进入 Interaction LLM；LLM 必须知道这是系统提醒，并用符合角色语气的文字提醒告知用户。一次性提醒完成，重复提醒推进下一次触发。 |
...
- 重复提醒是个人提醒核心合同的一部分；提醒运行层必须能根据规则推进下一次触发。

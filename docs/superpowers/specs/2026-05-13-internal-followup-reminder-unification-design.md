...
```text
origin: user | agent | web | calendar
...
```python
origin: Literal["user", "agent", "web", "calendar"]
...
- `metadata` is for runtime bookkeeping only, such as no-reply follow-up
  counters. Do not put user-facing task fields, goal fields, or arbitrary
  product state in it.
...
conversation, character, and route resolution.

Lifecycle after successful follow-up fire:

- one-shot internal follow-up completes after a successful agent output
- recurring internal follow-up is out of scope for this unification unless an
  existing caller already supplies an RRULE; do not add new recurrence behavior
  for follow-up in this change
...
Existing active `deferred_actions.kind=proactive_followup` rows may be handled
by a one-time cleanup script or ignored until expiry if they are not active in
production. The implementation plan must choose one path based on live/local
data inspection before deletion.

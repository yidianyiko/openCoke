...
class MongoDBBase:
    ...
    def get_vectors_by_text(
        self, collection_name: str, field: str, text: str
    ) -> List[Dict]:
        """
        通过文本内容获取向量文档
        field: "key" 或 "value"
    ...
    def update_metadata(
        self, collection_name: str, doc_id: str, metadata_updates: Dict[str, Any]
    ) -> bool:
        """
        更新文档的metadata字段
        可以只更新特定的metadata键
        ...
        result = self.db[collection_name].update_one(
            {"_id": ObjectId(doc_id)}, {"$set": update_dict}
        )
    ...
    def vector_search(
        ...
        if embedding_field not in ["key_embedding", "value_embedding"]:
            raise ValueError(
                "embedding_field must be 'key_embedding' or 'value_embedding'"
            )
        ...
        for doc in cursor:
            if embedding_field in doc and isinstance(doc[embedding_field], list):
                similarity = self._cosine_similarity(
                    query_embedding, doc[embedding_field]
                )
...
class VectorDB(MongoDBBase):
    ...
    def create_vector_collection(
        self, collection_name: str, vector_field: str = "vector"
    ):
        """创建向量集合并建立索引"""
    ...
    def insert_vector_document(
        ...
        document = {vector_field: vector, **metadata}

        return self.insert_one(collection_name, document)

    def insert_many_vector_documents(
        self,
        collection_name: str,
                ...
                "$search": {
                    ...
                    "compound": {
                        "should": [
                            ...
                            {"text": {"query": text_query, "path": text_field}},
                        ],
                        "score": {
                            "weights": {"knnBeta": vector_weight, "text": text_weight}
                        },
                    },
                }
            },
    ...
    def create_hybrid_index(
        ...
        self.db.command(
            {"createSearchIndexes": collection_name, "indexes": [index_config]}
        )
    ...
    def upsert_vector_document(
        ...
        update_data = {"$set": {vector_field: vector, **metadata}}

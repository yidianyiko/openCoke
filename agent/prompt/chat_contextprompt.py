...
【注意】消息中可能存在有问题的时间信息，请以实际的系统时间为准.
...
def get_relevant_history_context(context_retrieve: dict) -> str:
    """
    ...
    if relevant_history and relevant_history.strip():
        return """### 相关历史对话（语义检索）
以下是与当前话题语义相关的过往对话：
{relevant_history}"""
    return ""
...
- 分钟级别（< 60分钟）的无限重复提醒：不支持，频率过高会导致服务被限制，用户可以使用时间段提醒（如"上午9点到下午6点每30分钟"），小时级别以上的重复提醒（每小时、每天等）：支持，默认最多提醒10次"""
    ...
    if tool_context:
        ...
        _frequency_rules = FREQUENCY_LIMIT_RULES if is_frequency_error else ""  # 保留以备将来使用

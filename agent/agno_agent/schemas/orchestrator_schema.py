...
class ContextRetrieveParams(BaseModel):
    ...
    character_setting_keywords: str = Field(
        default="", description="角色设定关键词，逗号分隔，每个词不超过4字"
    )

    user_profile_query: str = Field(default="", description="用户资料检索语句")

    user_profile_keywords: str = Field(
        default="", description="用户资料关键词，逗号分隔"
    )

    character_knowledge_query: str = Field(default="", description="角色知识检索语句")

    character_knowledge_keywords: str = Field(
        default="", description="角色知识关键词，逗号分隔"
    )

    # Chat history retrieval
    chat_history_query: str = Field(
        default="", description="历史对话检索语句，用于找回与当前话题相关的过往对话"
    )

    chat_history_keywords: str = Field(
        default="", description="历史对话关键词，逗号分隔"
    )
...
class OrchestratorResponse(BaseModel):
    ...
    inner_monologue: str = Field(
        default="", description="角色的内心独白，理解用户意图的思考过程"
    )
    ...
    context_retrieve_params: ContextRetrieveParams = Field(
        default_factory=ContextRetrieveParams, description="上下文检索参数"
    )

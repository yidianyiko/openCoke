...
class ProactiveMessageTriggerService:
   ...
   -6.4: 更新会话的 future 状态
    ...
    def check_and_trigger(self) -> List[Dict[str, Any]]:
        ...
        logger.info(
            f"Found {len(due_conversations)} due conversations for proactive messages"
        )
        ...
        for conversation in due_conversations:
            try:
                ...
                logger.error(
                    f"Failed to trigger proactive message for conversation {conversation.get('_id')}: {e}"
                )
                results.append(
                    {
                        "conversation_id": str(conversation.get("_id", "")),
    ...
    def _trigger_proactive_message(
        ...
        if not user_id or not character_id:
            raise ValueError(
                f"Could not identify user and character from conversation {conversation_id}"
            )
        ...
        relation = self.mongo.find_one(
            "relations", {"uid": user_id, "cid": character_id}
        )

        if not user or not character:
            raise ValueError(
                f"User or character not found: user_id={user_id}, character_id={character_id}"
            )

        # 构建 session_state
        session_state = self._build_session_state(
            user, character, conversation, relation
        )
        ...
        self._update_conversation_future(
            conversation_id=conversation_id, updated_session_state=updated_session_state
        )

        return {"multimodal_responses": multimodal_responses, "future_updated": True}
    ...
    def _update_conversation_future(
        self, conversation_id: str, updated_session_state: Dict[str, Any]
    ) -> None:
        """
        更新会话的 future 状态
        ...
        """
        future_info = (
            updated_session_state.get("conversation", {})
        ...
        update_data = {
            ...
            "conversation_info.future.proactive_times": future_info.get(
                "proactive_times", 0
            ),
        }

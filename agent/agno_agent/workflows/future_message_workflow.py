...
class FutureMessageWorkflow:
    ...
    def _safe_render_template(
        self, template: str, session_state: Dict[str, Any]
    ) -> str:
        """
        安全地渲染模板字符串
        ...
        for key, value in session_state.items():
            if isinstance(value, str):
                ...
                if key == "conversation":
                    ...
                    flat_context["chat_history"] = str(
                        conv_info.get("chat_history", [])
                    )
                    flat_context["input_messages_str"] = conv_info.get(
                        "input_messages_str", ""
                    )
                    ...
                    flat_context["character_name"] = (
                        value.get("platforms", {}).get("wechat", {}).get("nickname", "")
                    )
                elif key == "user":
                    flat_context["user_name"] = (
                        value.get("platforms", {}).get("wechat", {}).get("nickname", "")
                    )
                elif key == "relation":
                    ...
                    flat_context["character_private"] = value.get(
                        "character_private", ""
                    )
                    flat_context["user_profile"] = value.get("user", "")
                    flat_context["character_knowledge"] = value.get(
                        "character_knowledge", ""
                    )
                else:
                    # 其他字典转为 JSON 字符串
    ...
    async def run(self, session_state: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        try:
            rendered_qr_userp = self._safe_render_template(
                self.query_rewrite_userp_template, session_state
            )
        except Exception as e:
        ...
        logger.debug(
            f"[FutureMessageWorkflow] QueryRewriteAgent LLM INPUT (len={len(rendered_qr_userp)}):\n{'='*50}\n{rendered_qr_userp}\n{'='*50}"
        )

        qr_response = await future_message_query_rewrite_agent.arun(
            input=rendered_qr_userp, session_state=session_state
        )
        ...
        cr_response = await future_message_context_retrieve_agent.arun(
            input=retrieve_message, session_state=session_state
        )
        ...
        try:
            rendered_chat_userp = self._safe_render_template(
                self.chat_userp_template, session_state
            )
        except Exception as e:
        ...
        logger.debug(
            f"[FutureMessageWorkflow] FutureMessageChatAgent LLM INPUT (len={len(rendered_chat_userp)}):\n{'='*50}\n{rendered_chat_userp}\n{'='*50}"
        )

        chat_response = await future_message_chat_agent.arun(
            input=rendered_chat_userp, session_state=session_state
        )
        ...
        if chat_response.content:
            ...
            session_state["MultiModalResponses"] = content.get(
                "MultiModalResponses", []
            )
        ...
        return {"content": content, "session_state": session_state}

    def _build_retrieve_message(self, query_rewrite: Dict, session_state: Dict) -> str:
        """构建上下文检索的消息"""
        # 这些变量从 session_state 提取，保留以备将来使用
        _character_id = session_state.get("character", {}).get("_id", "")
        _user_id = session_state.get("user", {}).get("_id", "")
...
"""
    ...
    def _handle_future_response(self, content: Dict, session_state: Dict) -> None:
        ...
        if new_proactive_times >= self.MAX_PROACTIVE_TIMES:
            ...
            logger.info(
                f"主动消息达到上限 ({new_proactive_times}/{self.MAX_PROACTIVE_TIMES})，设置为过期状态"
            )
        else:
            # 未达上限，可以继续规划下一次主动消息

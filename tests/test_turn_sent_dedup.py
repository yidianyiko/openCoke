...
class TestFullDedupFlow(unittest.TestCase):
    ...
    def test_full_dedup_scenario(self):
        ...
        sent_contents = [msg.get("message", "") for msg in round1_messages if msg.get("message")]
        existing = conversation["conversation_info"].get("turn_sent_contents", [])
        conversation["conversation_info"]["turn_sent_contents"] = existing + sent_contents
        
        self.assertEqual(conversation["conversation_info"]["turn_sent_contents"], ["哈哈"])

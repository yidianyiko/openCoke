...
def future_response_strategy(draw):
    ...
    return {
        "FutureResponseTime": draw(
            st.sampled_from(
                ["2025年12月25日09时00分", "2025年01月01日10时30分", "", "无效时间"]
            )
        ),
...
class TestRelationChangeBoundary(unittest.TestCase):
    ...
    @given(initial_rel=initial_relation_strategy(), change=relation_change_strategy())
...
class TestProactiveTimesIncrement(unittest.TestCase):
    ...
    def test_property_5_proactive_times_increment(self, initial_times):
        ...
        result_times = session_state["conversation"]["conversation_info"]["future"][
            "proactive_times"
        ]

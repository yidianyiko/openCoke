...
def future_message_response_strategy(draw):
    ...
    return {
        ...
        "FutureResponse": {
            "FutureResponseTime": draw(
                st.sampled_from(
                    ["2025年12月25日09时00分", "2025年01月01日10时30分", ""]
                )
            ),
            "FutureResponseAction": draw(
                st.sampled_from(["发送问候", "检查进度", "无"])
            ),
        },
    }
...
class TestWorkflowEndToEnd(unittest.TestCase):
    ...
    def test_workflow_end_to_end_returns_correct_structure(self):
        ...
        with (
            ...
            mock_chat_agent.run.return_value = self._create_mock_agent_response(
                mock_chat_response
            )
    ...
    def test_workflow_end_to_end_updates_relation(self):
        ...
        with (
            patch.object(self.workflow, "query_rewrite_userp_template", "测试"),
            patch.object(self.workflow, "chat_userp_template", "测试"),
            patch(
                "agent.agno_agent.workflows.future_message_workflow.future_message_query_rewrite_agent"
            ) as mock_qr_agent,
            patch(
    ...
    def test_workflow_end_to_end_updates_proactive_times(self):
        ...
        future_info = result["session_state"]["conversation"]["conversation_info"][
            "future"
        ]
...
class TestFrequencyControlIntegration(unittest.TestCase):
    ...
    def test_high_proactive_times_rarely_triggers(self):
        ...
        for _ in range(iterations):
            ...
            if (
                session_state["conversation"]["conversation_info"]["future"]["action"]
                is not None
            ):
                hits += 1
...
class TestTriggerServiceIntegration(unittest.TestCase):
    ...
    def test_full_trigger_flow(self):
        ...
        mock_workflow_result = {
            "content": {
                "InnerMonologue": "想要问候",
                "MultiModalResponses": [{"type": "text", "content": "你好呀！"}],
                "ChatCatelogue": "否",
                "RelationChange": {"Closeness": 1, "Trustness": 0},
                "FutureResponse": {
                    "FutureResponseTime": "",
    ...
    def test_trigger_flow_handles_multiple_conversations(self):
        ...
        mock_workflow_result = {
            "content": {"MultiModalResponses": [{"type": "text", "content": "你好"}]},
            "session_state": {
                "conversation": {
                    "conversation_info": {
                        "future": {

...
class TestQueryRewriteResponseSchema(unittest.TestCase):
    ...
    def test_model_dump(self):
        """测试 model_dump 输出"""
        response = QueryRewriteResponse(
            InnerMonologue="测试", CharacterSettingQueryQuestion="问题"
        )
...
class TestChatResponseSchema(unittest.TestCase):
    ...
    def test_default_values(self):
        ...
        self.assertFalse(
            hasattr(response, "RelationChange") and response.RelationChange is not None
        )
        self.assertFalse(
            hasattr(response, "FutureResponse") and response.FutureResponse is not None
        )
    ...
    def test_full_chat_response(self):
        ...
        response = ChatResponse(
            ...
            MultiModalResponses=[
                ...
                MultiModalResponse(
                    type="voice", content="很高兴认识你", emotion="高兴"
                ),
            ],
            ChatCatelogue="日常问候",
            # V2 重构：RelationChange 和 FutureResponse 已移至 PostAnalyzeResponse
...
class TestPostAnalyzeResponseSchema(unittest.TestCase):
    ...
    def test_model_dump(self):
        """测试 model_dump 输出"""
        response = PostAnalyzeResponse(
            UserRealName="测试用户", RelationDescription="好友"
        )
        data = response.model_dump()
        self.assertIsInstance(data, dict)

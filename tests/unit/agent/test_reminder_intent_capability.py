...
@pytest.mark.asyncio
async def test_reminder_intent_port_returns_explicit_today_past_clock_failure():
    from agent.agno_agent.capabilities.reminder_intent import ReminderIntentPort
    ...
    run_context = AgentRunContext(
        ...
        platform="business",
        recent_chat_history="",
        current_time=datetime(2026, 5, 25, 23, 56, tzinfo=UTC),
    )
    primary_decision = SimpleNamespace(
        intent_type="crud",
        action="create",
        title="喝水",
        trigger_at="2026-05-26T22:51:00+08:00",
    )
    ...
    result = await ReminderIntentPort(
        detector_agent=PrimaryAgent(),
        command_executor=executor,
    ).run("提醒我今天 06:51 喝水。", run_context)

    assert executor.received == []
    _assert_failed(result, "InvalidSchedule")
    assert "未来" in result.error.message


@pytest.mark.asyncio
async def test_reminder_intent_port_returns_runtime_local_today_past_clock_failure():
    from agent.agno_agent.capabilities.reminder_intent import ReminderIntentPort
    ...
    run_context = AgentRunContext(
        ...
        platform="business",
        recent_chat_history="",
        current_time=datetime(2026, 5, 25, 22, 8, tzinfo=UTC),
    )
    primary_decision = SimpleNamespace(
        intent_type="crud",
        action="create",
        title="喝水",
        trigger_at="2026-05-25T23:04:00+00:00",
    )
    ...
    class FakeExecutor:
        def execute(self, received_decision, run_context):
            raise AssertionError("runtime-local explicit past reminders must not execute")

    result = await ReminderIntentPort(
        detector_agent=PrimaryAgent(),
        command_executor=FakeExecutor(),
    ).run("提醒我今天 07:04 喝水。", run_context)

    _assert_failed(result, "InvalidSchedule")


@pytest.mark.asyncio
async def test_reminder_intent_port_returns_yesterday_past_clock_failure_without_executor():
    from agent.agno_agent.capabilities.reminder_intent import ReminderIntentPort
    ...
    primary_decision = SimpleNamespace(
        ...
        action="create",
        title="开会",
        trigger_at="2026-05-05T10:00:00+00:00",
    )
    ...
    class FakeExecutor:
        def execute(self, received_decision, run_context):
            raise AssertionError("explicit past reminders must not execute")

    result = await ReminderIntentPort(
        detector_agent=PrimaryAgent(),
        command_executor=FakeExecutor(),
    ).run("提醒我昨天 10 点开会。", _run_context())

    _assert_failed(result, "InvalidSchedule")
    assert result.operations[0].effect == "none"


@pytest.mark.asyncio
@pytest.mark.parametrize("text", ["明早提醒我喝水。", "下下周三提醒我喝水。"])
async def test_reminder_intent_port_rejects_vague_date_without_clock(text):
    from agent.agno_agent.capabilities.reminder_intent import ReminderIntentPort
    ...
    primary_decision = SimpleNamespace(
        ...
        action="create",
        title="喝水",
        trigger_at="2026-05-27T07:00:00+08:00",
    )
    ...
    class FakeExecutor:
        def execute(self, received_decision, run_context):
            raise AssertionError("date-only reminders must ask for a clock")

    result = await ReminderIntentPort(
        detector_agent=PrimaryAgent(),
        command_executor=FakeExecutor(),
    ).run(text, _run_context())

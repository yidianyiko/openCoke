...
async def test_handle_message_marks_stream_provider_error_for_rollback(
    ...
    class _PrepareWorkflow:
        async def run(self, *args, **kwargs):
            return {}

    class _PostAnalyzeWorkflow:
        async def run(self, *args, **kwargs):
            return {}

    class _StreamingChatWorkflow:
    ...
    monkeypatch.setitem(
        ...
        types.SimpleNamespace(
            PrepareWorkflow=_PrepareWorkflow,
            PostAnalyzeWorkflow=_PostAnalyzeWorkflow,
        ),
    )
    monkeypatch.setitem(
        sys.modules,
        "agent.agno_agent.workflows.chat_workflow_streaming",
        types.SimpleNamespace(StreamingChatWorkflow=_StreamingChatWorkflow),
    )
    monkeypatch.setitem(
        sys.modules,

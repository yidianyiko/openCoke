...
async def test_reminder_detect_timeout_range_fallback_uses_range_start_and_title(monkeypatch):
    ...
    orchestrator_response.metrics = None
    orchestrator_response.content.model_dump.return_value = {
        "inner_monologue": "schedule reminder",
        "need_context_retrieve": False,
        "context_retrieve_params": {},
        "need_reminder_detect": True,
        "need_web_search": False,
    ...
    operations = fallback_calls[0]["operations"]
    assert [(item["title"], item["trigger_at"][11:19]) for item in operations] == [
        ("起床", "06:30:00"),
        ("数学网课", "07:00:00"),
        ("数学练习", "09:10:00"),
    ]
    assert "-" not in [item["title"] for item in operations]
...
@pytest.mark.asyncio
async def test_reminder_detect_timeout_uses_deadline_fallback(monkeypatch):
    from agent.agno_agent.tools.tool_result import append_tool_result
    from agent.agno_agent.workflows import prepare_workflow
    from agent.agno_agent.workflows.prepare_workflow import PrepareWorkflow
    ...
    orchestrator_response.metrics = None
    orchestrator_response.content.model_dump.return_value = {
        "inner_monologue": "deadline reminder",
        "need_context_retrieve": False,
        "context_retrieve_params": {},
        "need_reminder_detect": True,
        "need_web_search": False,
...
@pytest.mark.asyncio
async def test_reminder_detect_timeout_uses_chinese_time_fallback(monkeypatch):
    from agent.agno_agent.tools.tool_result import append_tool_result
    from agent.agno_agent.workflows import prepare_workflow
    from agent.agno_agent.workflows.prepare_workflow import PrepareWorkflow
    ...
    with (
        ...
        result = await workflow.run("提醒我今天晚上九点30分乐跑", session_state)

    assert fallback_calls
    assert fallback_calls[0]["action"] == "create"
    assert fallback_calls[0]["title"] == "乐跑"
    assert "T21:30:00" in fallback_calls[0]["trigger_at"]
    assert fallback_calls[0]["rrule"] is None
    [tool_result] = result["session_state"]["tool_results"]
    assert tool_result["ok"] is True
...
@pytest.mark.asyncio
async def test_reminder_detect_timeout_uses_implicit_colon_time_fallback(monkeypatch):
    from agent.agno_agent.tools.tool_result import append_tool_result
    from agent.agno_agent.workflows import prepare_workflow
    from agent.agno_agent.workflows.prepare_workflow import PrepareWorkflow
    ...
    orchestrator_response.metrics = None
    orchestrator_response.content.model_dump.return_value = {
        "inner_monologue": "implicit reminder",
        "need_context_retrieve": False,
        "context_retrieve_params": {},
        "need_reminder_detect": True,
        "need_web_search": False,

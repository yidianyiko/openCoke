...
"""
        ...
        # 验证必需字段存在
        assert "InnerMonologue" in content_dict or hasattr(
            response.content, "InnerMonologue"
        )
...
"""
        ...
        # 验证必需字段存在
        assert "MultiModalResponses" in content_dict or hasattr(
            response.content, "MultiModalResponses"
        )
        ...
        if content:
            ...
            if responses:
                for resp in responses:
                    ...
                    logger.info(
                        f"消息类型: {resp.get('type')}, 内容: {resp.get('content', '')[:50]}..."
                    )

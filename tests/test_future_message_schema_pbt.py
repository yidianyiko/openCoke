...
def relation_change_strategy(draw):
    ...
    closeness = draw(st.floats(min_value=-100, max_value=100, allow_nan=False, allow_infinity=False))
    trustness = draw(st.floats(min_value=-100, max_value=100, allow_nan=False, allow_infinity=False))
...
def future_response_strategy(draw):
    ...
    return FutureResponseModel(FutureResponseTime=time_str, FutureResponseAction=action_str)
...
def future_message_response_strategy(draw):
    ...
    multimodal_responses = draw(st.lists(multimodal_response_strategy(), min_size=0, max_size=5))
...
class TestFutureMessageResponseSchemaProperty(unittest.TestCase):
    ...
    @given(future_message_response_strategy())
    @settings(max_examples=100)
    def test_property_1_2_future_response_subfields(self, response: FutureMessageResponse):
        """
    ...
    @given(future_message_response_strategy())
    @settings(max_examples=100)
    def test_property_1_3_multimodal_response_types(self, response: FutureMessageResponse):
        """

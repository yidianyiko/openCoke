...
class TestContextPrepareImpact(unittest.TestCase):
    ...
    def test_context_prepare_converts_all_objectids(self):
        ...
        self.assertIsInstance(converted["conversation"]["conversation_info"]["input_messages"][0]["_id"], str)
...
class TestMongoDBIntegration(unittest.TestCase):
    ...
    def test_replace_one_with_objectid_query_and_update(self):
        ...
        # 读取数据
        result = self.mongo.find_one(self.test_collection, {"_id": ObjectId(inserted_id)})
        ...
        # 验证数据已更新
        updated = self.mongo.find_one(self.test_collection, {"_id": ObjectId(inserted_id)})

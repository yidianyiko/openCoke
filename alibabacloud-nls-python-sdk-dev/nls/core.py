...
from .exception import ConnectionTimeout, ConnectionUnavailable, InvalidParameter
...
class NlsCore:
    ...
    def __run(self, ping_interval, ping_timeout):
        ...
        self.__ws.run_forever(ping_interval=ping_interval, ping_timeout=ping_timeout)
        with self.__lock:
            self.__connection_status = NlsConnectionStatus.Disconnected
    ...
    def __connect_before_start(self, ping_interval, ping_timeout):
        with self.__cond:
            self.__th = threading.Thread(
                target=self.__run, args=[ping_interval, ping_timeout]
            )
            self.__th.start()
            if self.__connection_status == NlsConnectionStatus.Disconnected:
                ...
                if not self.__async:
                    if self.__cond.wait(timeout=10):
                        ...
                        raise ConnectionTimeout(
                            "Wait response timeout! Please check local network!"
                        )

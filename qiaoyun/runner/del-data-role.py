...
def delete_user_completely(user_identifier: str):
    ...
    relations_count = mongo.count_documents("relations", {"uid": wechat_id}) if wechat_id else 0
    conversations_count = mongo.count_documents("conversations", {"talkers.id": wechat_id}) if wechat_id else 0
    input_from_count = mongo.count_documents("inputmessages", {"from_user": wechat_id}) if wechat_id else 0
    input_to_count = mongo.count_documents("inputmessages", {"to_user": wechat_id}) if wechat_id else 0
    ...
    mod_convs = 0
    if wechat_id:
        mod_convs = mongo.update_many("conversations", {}, {"$pull": {"talkers": {"id": wechat_id}}})

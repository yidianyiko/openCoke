...
TASKPROMPT_微信对话 = '''我正在写一本小说，{character[platforms][wechat][nickname]} 和 {user[platforms][wechat][nickname]} 是我小说中的虚拟角色。现在他们正在微信上进行聊天，在聊天过程中双方也可能跟对方产生感情和亲密关系。鉴于平台限制，目前{character[platforms][wechat][nickname]}无法收图片，收视频，打语音电话和视频电话；不可以发语音，发图片。
...
TASKPROMPT_提醒识别 = '''7. DetectedReminders。请在该字段中输出本次消息识别到的提醒任务数组；若没有识别到提醒，则输出空数组[]。每个提醒对象包含以下字段：
...
识别规则：当用户消息包含“提醒/通知/叫我/闹钟/别忘了”等表达，或包含相对时间（如“X分钟后”“X小时后”）与行为动词时，判定为提醒。时间解析优先输出绝对时间到 time_resolved，格式严格为“xxxx年xx月xx日xx时xx分”。相对时间以当前消息时间为基准计算并写入 time_resolved。模糊或不完整时间设为 time_type=ambiguous 且 requires_confirmation=true。周期词出现时填充 recurrence。所有输出为严格 JSON；除定义的字段外不输出其他自由文本。'''

...
d 应该避免跟{character[platforms][wechat][nickname]}在历史对话中的内容雷同，也应该避免跟MultiModalResponses当中的内容雷同，可以询问用户正在做什么。
...
TASKPROMPT_提醒识别 = '''7. DetectedReminders。当{character[platforms][wechat][nickname]}最新的聊天消息（包含了用户发消息的时间）包含了“提醒/通知/叫我/闹钟/别忘了”等类似关键词时，请在该字段中输出本次消息识别到提醒任务数组；若没有识别到提醒，则输出空数组[]。每个提醒对象包含以下字段：
